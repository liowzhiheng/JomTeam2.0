...
interface HostedRequest { id:string; matchTitle:string; playerName:string; initials:string;avatarPath:string|null }
...
@Component({selector:'app-dashboard-page',imports:[RouterLink],template:`
  ...
  <div class="metric-grid"><div><span>Pending requests</span><strong>{{requests().length}}</strong><small>Waiting for your response</small></div><a class="metric-link" routerLink="/matches" [queryParams]="{mine:'true'}" aria-label="View all matches created by you"><span>Created matches</span><strong>{{stats().created}}</strong><small>Hosted by you <b aria-hidden="true">→</b></small></a><a class="metric-link" routerLink="/friends" aria-label="View all friends"><span>Friends</span><strong>{{stats().friends}}</strong><small>Your sports community <b aria-hidden="true">→</b></small></a><div class="activity-metric"><span class="activity-ring" [style.--progress]="completionProgress()+'%'"><b>{{stats().completed}}</b></span><span><strong>Matches played</strong><small>Completed activities</small></span></div></div>
  <section class="form-card"><div class="section-heading"><h2>Pending join requests</h2><a routerLink="/matches">View matches</a></div>
    ...
    @else if(requests().length){@for(request of requests();track request.id){<div class="request-row"><span class="avatar">@if(avatarUrl(request.avatarPath);as url){<img [src]="url" [alt]="request.playerName+' profile photo'"/>}@else{{{request.initials}}}</span><span><strong>{{request.playerName}}</strong><small>Wants to join {{request.matchTitle}}</small></span><div><button class="button secondary" type="button" [disabled]="processing()===request.id" (click)="decline(request)">{{processing()===request.id?'Working…':'Decline'}}</button><button class="button primary" type="button" [disabled]="processing()===request.id" (click)="accept(request)">{{processing()===request.id?'Working…':'Accept'}}</button></div></div>}}
...
export class DashboardPage implements OnInit{
  ...
  avatarUrl(path:string|null|undefined){return this.images.publicUrl('avatars',path);}
}
