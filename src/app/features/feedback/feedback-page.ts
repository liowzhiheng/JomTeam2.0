...
@Component({selector:'app-feedback-page',imports:[ReactiveFormsModule,DatePipe],template:`
  ...
  <div class="editor-layout"><form class="form-card feedback-form" [formGroup]="form" (ngSubmit)="submit()"><h2>Send feedback</h2><label>Title<input formControlName="title" maxlength="120" placeholder="What would you like us to know?" /></label><label>Rating<select class="rating-control" formControlName="rating"><option [ngValue]="5">5 — Excellent</option><option [ngValue]="4">4 — Good</option><option [ngValue]="3">3 — Okay</option><option [ngValue]="2">2 — Needs work</option><option [ngValue]="1">1 — Poor</option></select></label><label>Description<textarea rows="6" formControlName="description" maxlength="2000" placeholder="Share enough detail to help us improve."></textarea></label>@if(message()){<div class="alert success" role="status">{{message()}}</div>}@if(error()){<div class="alert error" role="alert">{{error()}}</div>}<button class="button primary" type="submit" [disabled]="submitting()">{{submitting()?'Sending…':'Send feedback'}}</button></form>
