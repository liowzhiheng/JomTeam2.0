  ...
  template: `
    <article class="match-card">
      ...
      <div class="match-body">
        <div class="host"><span class="avatar small">@if(hostAvatarUrl()){<img [src]="hostAvatarUrl()!" [alt]="match().hostName+' profile photo'"/>}@else{{{ initials(match().hostName) }}}</span><span>Hosted by <strong>{{ match().hostName }}</strong></span></div>
...
export class MatchCard { private readonly images=inject(ImageService);readonly match=input.required<MatchSummary>();readonly icons={calendar:faCalendarDays,location:faLocationDot,users:faUsers};readonly seats=()=>Array.from({length:Math.min(this.match().maxPlayers,8)});hostAvatarUrl(){return this.images.publicUrl('avatars',this.match().hostAvatarUrl);}coverImage(){const url=this.images.publicUrl('match-covers',this.match().coverUrl);return url?`linear-gradient(rgba(8,35,65,.18),rgba(8,35,65,.18)), url("${url}")`:null;}initials(name:string){return name.split(' ').map(p=>p[0]).slice(0,2).join('');} }
